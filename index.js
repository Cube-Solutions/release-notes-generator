...
export async function generateNotes(pluginConfig, context) {
    ...
    const extendedCommits = [...parsedCommits] // to convert from Generator to Array
    const {extensions} = pluginConfig;
    if (extensions) {
        for (let extensionConfig of extensions) {
            debug('Applying extension: %s', extensionConfig.name);

            const extension = await importFrom.silent(__dirname, extensionConfig.name) || await importFrom(cwd, extensionConfig.name);
            await extension(extendedCommits, extensionConfig.config, pluginConfig, context)
        }
    }
    ...
    return getStream(intoStream.object(extendedCommits).pipe(writer(changelogContext, writerOpts)));
}
